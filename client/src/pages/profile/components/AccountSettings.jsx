...
const AccountSettings = () => {
  ...
  const handleSubmit = async (e) => {
    e.preventDefault();
    try {
      const response = await axios.patch(`http://localhost:8000/api/changePassword/${user?._id}`, formData);
      if(response.data){
  ...
  const editHandleSubmit=async(e)=>{
    e.preventDefault();
    try {
      const response = await axios.patch(`http://localhost:8000/api/editProfile/${user?._id}`, profileFormData);
      if(response.data){
